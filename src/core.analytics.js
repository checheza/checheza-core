...
export default class CoreAnalytics {
	initialize() {
		...
		Core.backend.checkConnection().then((hasConnection) => {
			if (hasConnection) {
	...
	dispatchData() {
		Core.database.getClicks().then((clicks) => {
			Core.log('Dispatching data to analytics server');
			Core.backend
				.POST('/report/clicks', { clicks: clicks })
				.then((response) => {
					if (response.success) {
	...
	createClickObject(event) {
		return {
			...
			y: event.clientY,
			targetElem:
				event.path[0].id !== undefined ? event.path[0].id : undefined,
		};
	}
